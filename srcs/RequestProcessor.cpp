...
/* 	std::string url = _parsedRequest.getUri();
    ...
    if (this->_endpoint == "/" || this->_endpoint == "/favicon.ico") {
        _response->setStatus(200);
        _response->setHeader("Content-Type", "text/html");
        _response->setBody(readFile("./var/www/html/index.html"));
        //_response->setHeader("Content-Length", _sizeToString(_response->getBody().length()));
    } else if (this->_endpoint.find(".html") != std::string::npos) {
        _response->setStatus(200); // this is not correct fully, it should be based on the file
        _response->setHeader("Content-Type", "text/html");
        _response->setBody(readFile("./var/www/html" + this->_endpoint));
        //_response->setHeader("Content-Length", _sizeToString(_response->getBody().length()));
...
/* HTTPResponse RequestProcessor::handlePOSTRequest(const HTTPRequest &req) {
    std::string url = _parsedRequest.getUri();
    ...
    if (this->_endpoint == "/user_create") {
        std::string res = _usersDB.addUserToDatabase(this->_queryParams);
        _response->setStatus(200);
        _response->setHeader("Content-Type", "text/html");
        _response->setBody(res);
    } else {
...
/* HTTPResponse RequestProcessor::handleDELETERequest(const HTTPRequest &req) {
    std::string url = _parsedRequest.getUri();
    parseURI(_parsedRequest.getUri());
    std::cout << "DELETE request for: " << url << std::endl;
    std::cout << "{" << url << "}" << std::endl;
    std::cout << "{" << this->_endpoint << "}" << std::endl;
    std::cout << "{" << this->_queryParams["username"] << "}" << std::endl;

    std::cout << "==============Print _queryParams================" << std::endl;
    for (std::map<std::string, std::string>::iterator it = _queryParams.begin(); it != _queryParams.end(); ++it) {
        std::cout << it->first << " => " << it->second << st
    response.setHeader("Content-Length", toString(size));d::endl;
    }
    std::cout << "==============Print _queryParams================" << std::endl;

    if (this->_endpoint == "/user") {
        std::string res = _usersDB.deleteUserFromDatabase(this->_queryParams["username"], _response);
        _response->setHeader("Content-Type", "text/html");
        _response->setBody(res);
    } 
    else {
